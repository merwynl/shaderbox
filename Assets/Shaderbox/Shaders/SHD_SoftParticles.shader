...
Shader "SHD_SoftParticles"
{
	Properties
	{
		_MainTex( "MainTex", 2D ) = "white" {}
		_FadeDistance( "FadeDistance", Float ) = 1
		_AlphaThreshold( "AlphaThreshold", Range( 0, 1 ) ) = 1
		_Glow( "Glow", Float ) = 0
	...
	LOD 0
		...
		Pass
		{
						 ...
						 ) : SV_Target
			{
				...
				float4 tex2DNode150 = tex2D( _MainTex, uv_MainTex );
				
				float screenDepth155 = LinearEyeDepth(SHADERGRAPH_SAMPLE_SCENE_DEPTH( ScreenPosNorm.xy ),_ZBufferParams);
				float distanceDepth155 = ( screenDepth155 - LinearEyeDepth( ScreenPosNorm.z,_ZBufferParams ) ) / ( _FadeDistance );
				
				float3 BakedAlbedo = 0;
				float3 BakedEmission = 0;
				float3 Color = ( tex2DNode150 * ( _Glow * input.ase_color ) ).rgb;
				float Alpha = ( ( tex2DNode150.a * input.ase_color ) * distanceDepth155 ).r;
		...
		Pass
		{
			...
			half4 frag(PackedVaryings input ) : SV_Target
			{
				...
				float screenDepth155 = LinearEyeDepth(SHADERGRAPH_SAMPLE_SCENE_DEPTH( ase_positionSSNorm.xy ),_ZBufferParams);
				float distanceDepth155 = ( screenDepth155 - LinearEyeDepth( ase_positionSSNorm.z,_ZBufferParams ) ) / ( _FadeDistance );
				

				surfaceDescription.Alpha = ( ( tex2DNode150.a * input.ase_color ) * distanceDepth155 ).r;
				#if defined( _ALPHATEST_ON )
...
WireConnection;138;2;153;0
WireConnection;138;3;161;0
WireConnection;138;4;157;0
ASEEND*/
//CHKSM=90251E81C8B47AAE17AF7C0F6CD2F876EF3A1B0D