Shader"Unlit/shd_clamping"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                // t values exceed 0-1, creating overshading
                // Use the saturate to clamp values. if v < 0, v = 0; if v > 1, v = 1;
                
                float t = saturate(InverseLerp(_ColorStart, _ColorEnd, i.uv.x));

                // frac = some value(v) - floor(v). Values will repeat in the 0-1 internal.
                // Use frac to check if values are repeating
                // t = frac(t);
